...
If `{story_key}` is not empty and `/home/alex/pets/jev-trade/_bmad-output/implementation-artifacts/sprint-status.yaml` exists, read `/home/alex/pets/jev-trade/_bmad/render/bmad-build/jev-trade-ca1ef59b338d/58d96c60437d5aa4fa91/sync-sprint-status.md` with `{target_status}` = `in-progress`.
...
**When to stop and replan.** Stop coding if the request left out something the user would notice in the result. Write the gap in `## Implementation Notes`. Then update `{spec_file}`: add back `## Code Map` (filled in from what you learned while implementing) and `## Open Questions` (one question per gap), set `route: 'full'` and `status: 'draft'`. Go back to `/home/alex/pets/jev-trade/_bmad/render/bmad-build/jev-trade-ca1ef59b338d/58d96c60437d5aa4fa91/step-02-plan.md` step 6.
...
If `{story_key}` is not empty and `/home/alex/pets/jev-trade/_bmad-output/implementation-artifacts/sprint-status.yaml` exists, read `/home/alex/pets/jev-trade/_bmad/render/bmad-build/jev-trade-ca1ef59b338d/58d96c60437d5aa4fa91/sync-sprint-status.md` with `{target_status}` = `review`.
